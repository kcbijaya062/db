...
document.getElementById("getFile").onchange = function () {
  let userFile = this.files[0]; // Corrected variable name: useFile -> userFile
  try {
    let isText = userFile.type.startsWith("text"); // Corrected variable name: Text -> text
    if (!isText) {
      throw userFile.name + " is not a text file"; // Added a space before "is not a text file"
    }
...
function wordCloud(sourceText) {
  ...
  // Remove non-alphabet characters and whitespace from the text
  let alphaRegex = /[^a-zA-Z\s]/g;
  sourceText = sourceText.replace(alphaRegex, "");
  ...
  // Find duplicate words and their counts
